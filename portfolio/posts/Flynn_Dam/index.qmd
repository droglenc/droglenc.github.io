...
dat <- readxl::read_excel("../_data/Flynn_Dam_Heights.xlsx",skip=3) |>
  mutate(DateTime2=case_when(
           ...
           Season=="2026-27" ~ DateTime %m-% years(3),
           TRUE ~ DateTime),
...
- Median water levels over the season appear to be around 22.0-23.0" (@fig-depth_distribution; @tbl-depth_summary).
...
- Water levels are dependent on rainfall (not surprisingly!). The levels at the start of the 2023-24 season were elevated due to near record snowfalls the previous winter, whereas the lowest recordings (thusfar) were in the middle of summer for the 2026-27 season when we experienced an extended periods of little to no rain and above-average temperatures (@fig-depth_series).
...
#  geom_line(aes(linewidth=currSeason),alpha=0.9) +
  ...
  scale_y_continuous(name="Water Level (in.)",expand=expansion(add=c(0.25,0.25)),
                     breaks=depvals) +
...
ggplot(data=dat,mapping=aes(y=Depth,x=Season,fill=Season)) +
  ...
  scale_y_continuous(name="Water Level (in.)",breaks=depvals) +
...
dMaxDD <- format(dat2$Date[dat2$dDepth==maxDD],format="%B %e, %Y")
```
...
ggplot() +
  ...
  geom_point(data=filter(dat2,!is.na(dat2$Rain)),
  ...
  scale_y_continuous(name="Change in Water Level from Previous Day (in)",
                     ...
                     limits=c(-1,2.6),breaks=dDepvals,labels=dDeplbls) +
  scale_fill_brewer(name="Precipitation:",palette="YlOrRd") +
  facet_wrap(vars(Season),ncol=1,strip.position="right") +
  scale_x_date(limits=as.Date(c("2023-3-14","2024-4-20"),format="%Y-%m-%d"),
                     date_breaks="1 month",
                     date_labels="%b",expand=expansion(mult=0.025)) + 
  labs(caption=tmp_source) +
  theme_dam()
