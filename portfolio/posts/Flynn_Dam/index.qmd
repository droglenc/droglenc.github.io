...
ggplot(data=dat,mapping=aes(x=DateTime,y=Depth)) +
  geom_line(linewidth=2,color="slategray") +
...
## Water Depth at Dam Relative to Recent Rain Events
...
ggplot(data=dat,mapping=aes(x=DateTime,y=Depth)) +
  ...
  geom_point(mapping=aes(color=Rain2),size=2.5) +
  geom_hline(yintercept=basedep,linetype="dashed",size=1) +
  annotate(geom="text",x=min(dat$DateTime),y=1.92,label="Summer Baseline",
           hjust=0.1,vjust=-0.5) +
  scale_x_datetime(date_breaks="1 week",date_minor_breaks="1 day",
  ...
  scale_y_continuous(name="Gauge Depth",expand=expansion(mult=0),
                     limits=range(depvals/12),breaks=depvals/12,labels=deplbls) +
  scale_color_brewer(name="Rain Amount",palette="YlOrRd") +
  theme_minimal(base_size=12) +
  theme(panel.grid.minor.y=element_blank(),
        panel.grid.minor.x=element_line(linetype="dashed"),
        axis.title.x=element_blank(),
