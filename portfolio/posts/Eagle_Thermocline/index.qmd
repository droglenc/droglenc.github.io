...
- The bottom of the thermocline is generally betwen 25-30 feet deep in mid to late summer.
:::
...
The dissolution of the thermocline in late fall was more evident in the data from 2020, where the temperature was almost constant throughout the water column on the last observation date of `r format(max(d20$Date),"%B %e")` (@fig-ex2020).


```{r}
#| label: fig-ex2020
#| fig-cap: !expr paste("Temperature profile plots for",xfun::numbers_to_words(length(unique(d20$Date))),"dates throughout the open-water season in 2020.")
#| out-width: 50%
tp(d20) +
  transition_states(Date,wrap=FALSE) +
  labs(title='{format(as.Date(closest_state),"%B %e, %Y")}') +
