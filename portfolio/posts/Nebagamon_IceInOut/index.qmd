...
- The range for both ice-in (`r sum$inEarlyDate` to `r sum$inLateDate`) and ice-out (`r sum$outEarlyDate` to `r sum$outLateDate`) dates is considerable, at approximately six weeks for each (see @fig-inouthist).
...
- Seven of the ten shortest ice duration seasons have occurred since 2000 (see @tbl-durShort), whereas only one of the ten longest ice duration seasons have occurred in that same time period (see @tbl-durLong).
