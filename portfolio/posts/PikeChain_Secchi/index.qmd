    ...
    theme(axis.text.x=element_text(angle=90,vjust=0.5),
          ...
          strip.text=element_text(color="white",face="bold",
                                  margin=margin(t=0.5,b=0.5,r=0.5,l=0.5,
                                                unit="mm")),
          plot.background=element_rect(fill="#FEFCFF"))
  p
}
```
...
wq <- readxl::read_excel("../_data/Pike_Chain_WQ_Data.xlsx",sheet="Secchi") |>
  select(lake=`Lake`,date=`Start Date`,secchi=`Secchi (Feet)`,
         hit_bottom=`Secchi Hit Bottom?`,chl=`Chlorophyll(ug/l)`,
         P=`Total Phosphorus(ug/l)`,TSI_secchi=`Secchi TSI`,
         TSI_chl=`Chlorophyll TSI`,TSI_P=`Total Phosphorus TSI`,
         lake_level=`Lake Level`,appearance=Appearance,color=Color,
...
## Introduction
The Pike Chain of Lakes near Iron River, WI is a set of seven connected lakes. The Pike Chain is highly developed, has several public access locations, and is popular among anglers, boaters, snowmobilers, etc. year-round. Over the years, especially recently since [Eurasian watermilfoil](https://www.invasivespeciesinfo.gov/aquatic/plants/eurasian-watermilfoil) (@fig-EagleMilfoil) was introduced, volunteers have recorded various water quality parameters for the Pike Chain of lakes. Below I show the record of Secchi depth recordings that are publicly available through the [Wisconsin DNR Citizen Lake Monitoring Network](https://dnr.wisconsin.gov/topic/lakes/clmn).

```{r}
#| label: fig-EagleMilfoil
#| fig-cap: Eurasion watermilfoil on Eagle Lake, early summer 2022.
#| out-width: 67%
knitr::include_graphics("Eagle_Milfoil.jpg")
```
...
```{r}
#| label: tbl-lakedata
#| tbl-cap: Characteristics of each lake in the Pike Chain of Lakes.
...
for (i in seq_along(lakeid)) {
  cat("- ",lakes[i],": [https://dnr.wi.gov/lakes/waterquality/Station.aspx?id=",
      lakeid[i],"](https://dnr.wi.gov/lakes/waterquality/Station.aspx?id=",
      lakeid[i],")\n",sep="")
}
```
